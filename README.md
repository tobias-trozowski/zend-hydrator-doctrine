...
[![Build Status](https://travis-ci.com/tobias-trozowski/zend-hydrator-doctrine.svg)](https://travis-ci.com/tobias-trozowski/zend-hydrator-doctrine)
...
This package ships with a very powerful hydrator that allow almost any use-case.
...
```php
use Tobias\Zend\Hydrator\Doctrine\DoctrineHydrator;

$hydrator = new DoctrineHydrator($objectManager);
```

A hydrator can be used for multiple objects.
...
`Tobias\Zend\Hydrator\Doctrine\Strategy\AbstractCollectionStrategy`.

This package provides four hydration strategies out of the box:

1. `Tobias\Zend\Hydrator\Doctrine\Strategy\AllowRemoveByValue`: this is the default strategy, it removes old elements that are not in the new collection.
2. `Tobias\Zend\Hydrator\Doctrine\Strategy\AllowRemoveByReference`: this is the default strategy (if set to byReference), it removes old elements that are not in the new collection.
3. `Tobias\Zend\Hydrator\Doctrine\Strategy\DisallowRemoveByValue`: this strategy does not remove old elements even if they are not in the new collection.
4. `Tobias\Zend\Hydrator\Doctrine\Strategy\DisallowRemoveByReference`: this strategy does not remove old elements even if they are not in the new collection.
